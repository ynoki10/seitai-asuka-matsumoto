...
const Footer = () => {
  return (
    <>
      <footer className={'py-10 mt-[100px] text-white bg-green03'}>
        <Inner>
          <div className={'mx-auto max-w-[225px] md:max-w-[280px]'}>
            <Link href="/">
              <a>
                <Image
          ...
          <p className={'mt-4 text-sm font-normal tracking-wider leading-relaxed text-center'}>
            〒390-1243
            <br />
            長野県松本市神林5108-8
            <br />
          ...
          <div className={'mt-6'}>
            ...
            <nav className={'flex flex-col gap-y-6 items-center mt-8'}>
              {footerMenuItems.map((item) => {
                return (
                  <Link href={item.path} key={item.text}>
                    <a className={'font-normal'}>{item.text}</a>
                  </Link>
                );
              })}
            </nav>
            <small className={'block mt-10 text-xxs text-center'}>
